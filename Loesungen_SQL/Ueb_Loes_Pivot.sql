...
from   (select product_code, quantity
        from   pivot_test)
pivot  (sum(quantity) as sum_quantity for (product_code)
...
from   (select customer_id, product_code, quantity
        from   pivot_test)
pivot xml (sum(quantity) as sum_quantity for (product_code) 
        in (select distinct product_code from  pivot_test)); 
...
select *
from   unpivot_test
unpivot (quantity for product_code in 
             (product_code_a as 'A', 
              product_code_b as 'B', 
              product_code_c as 'C', 
              product_code_d as 'D'));
